...
import { PortfolioTherapist } from './PortfolioTherapist';
...
function TherapistPage({ token, userId, role }: { token: string; userId: string; role: string }) {
  return <AppShell title="Portfolio Therapist" token={token} role={role} logout={() => localStorage.clear()} connectionStatus="connected"><PortfolioTherapist token={token} userId={userId} /></AppShell>;
}
