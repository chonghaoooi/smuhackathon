...
const DEFAULT_COMPANY_PROFILES: Record<string, { hq: string; founded: string; ceo: string; focus: string; scenario: string; news: string }> = {
  AUR: {
    hq: 'Austin, Texas',
    founded: '2018',
    ceo: 'Maya Chen',
...
function authFromReq(req: any): { sub: string; role: Role } | null {
...
function requireRole(role: Role) {
  return (req: any, res: any, next: any) => {
    ...
    if (!auth || auth.role !== role) return res.status(401).json({ error: 'Unauthorized' });
    req.auth = auth;
    next();
  };
}

function teamNetWorth(teamId: string) {
  const team = db.prepare('SELECT id, name, cash FROM teams WHERE id = ?').get(teamId) as any;
  if (!team) return null;
...
function writeAdminLog(action: string, details: unknown) {
  db.prepare('INSERT INTO admin_logs (id, action, details) VALUES (?, ?, ?)').run(id(), action, JSON.stringify(details));
}

function presenceKey(userId: string, role: Role) {
  return `${role}:${userId}`;
}

function upsertPresence(userId: string, role: Role, sessionKey = presenceKey(userId, role)) {
  db.prepare(`
...
function restoreSnapshotPayload(payload: any) {
  db.transaction(() => {
    for (const table of ['teams', 'companies', 'holdings', 'transactions', 'articles', 'article_company_relations', 'notifications', 'quizzes', 'quiz_attempts', 'market_settings', 'market_events', 'watchlists', 'team_networth_history', 'admin_logs']) {
      if (table === 'market_settings') continue;
      db.prepare(`DELETE FROM ${table}`).run();
    }
    for (const row of payload.teams ?? []) db.prepare('INSERT INTO teams (id, name, username, password_hash, cash, created_at) VALUES (?, ?, ?, ?, ?, ?)').run(row.id, row.name, row.username, row.password_hash, row.cash, row.created_at);
    for (const row of payload.companies ?? []) db.prepare('INSERT INTO companies (id, name, ticker, industry, description, revenue, profit, debt, dividend_yield, price, volume, sector, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)').run(row.id, row.name, row.ticker, row.industry, row.description, row.revenue, row.profit, row.debt, row.dividend_yield, row.price, row.volume, row.sector, row.created_at);
    for (const row of payload.holdings ?? []) db.prepare('INSERT INTO holdings (id, team_id, company_id, quantity, avg_price) VALUES (?, ?, ?, ?, ?)').run(row.id, row.team_id, row.company_id, row.quantity, row.avg_price);
    for (const row of payload.transactions ?? []) db.prepare('INSERT INTO transactions (id, team_id, company_id, action, quantity, price, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)').run(row.id, row.team_id, row.company_id, row.action, row.quantity, row.price, row.created_at);
    for (const row of payload.articles ?? []) db.prepare('INSERT INTO articles (id, title, body, category, impact_type, published, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)').run(row.id, row.title, row.body, row.category, row.impact_type, row.published, row.created_at);
    for (const row of payload.notifications ?? []) db.prepare('INSERT INTO notifications (id, team_id, title, body, type, priority, pinned, read_at, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)').run(row.id, row.team_id, row.title, row.body, row.type, row.priority, row.pinned, row.read_at, row.created_at);
    for (const row of payload.quizzes ?? []) db.prepare('INSERT INTO quizzes (id, question, options_json, answer_index, reward, active, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)').run(row.id, row.question, row.options_json, row.answer_index, row.reward, row.active, row.created_at);
    const market = payload.market_settings;
    if (market) db.prepare('UPDATE market_settings SET status = ?, event_mode = ?, inflation = ?, interest_rate = ?, updated_at = CURRENT_TIMESTAMP WHERE id = 1').run(market.status, market.event_mode, market.inflation, market.interest_rate);
  })();
}

function maybeAutoSnapshot(action: string) {
  const settings = db.prepare('SELECT event_mode FROM market_settings WHERE id = 1').get() as any;
  if (!settings?.event_mode) return;
  try {
...
function notifyWatchlistedTeams(companyIds: string[], title: string, body: string, type: 'news' | 'notification' = 'news') {
  const uniqueTeamIds = new Set<string>();
  for (const companyId of companyIds) {
    const teams = db.prepare('SELECT team_id FROM watchlists WHERE company_id = ?').all(companyId) as Array<{ team_id: string }>;
    for (const row of teams) uniqueTeamIds.add(row.team_id);
  }
...
function companyNamesFromIds(companyIds: string[]) {
  if (!companyIds.length) return [];
  const placeholders = companyIds.map(() => '?').join(', ');
  const rows = db.prepare(`SELECT id, name, ticker FROM companies WHERE id IN (${placeholders})`).all(...companyIds) as Array<{ id: string; name: string; ticker: string }>;
  const byId = new Map(rows.map((row) => [row.id, row]));
  return companyIds.map((companyId) => byId.get(companyId)).filter(Boolean) as Array<{ id: string; name: string; ticker: string }>;
}

function formatCompanyMentions(companyIds: string[]) {
...
function enrichArticleRow(article: any) {
  ...
  `).all(article.id) as Array<{ id: string; name: string; ticker: string }>;
...
function upsertCompanyProfile(companyId: string, profile: { hq?: string; founded?: string; ceo?: string; focus?: string; scenario?: string; news?: string }) {
  db.prepare(`
...
export function seedCompanyProfiles() {
  const companies = db.prepare('SELECT id, ticker FROM companies').all() as Array<{ id: string; ticker: string }>;
  for (const company of companies) {
    const profile = DEFAULT_COMPANY_PROFILES[company.ticker];
    if (profile) upsertCompanyProfile(company.id, profile);
  }
}

export function createApi(io: Server) {
  seedCompanyProfiles();
  const router = Router();
  ...
  router.post('/auth/login', async (req, res) => {
    ...
    const admin = db.prepare('SELECT id, username, password_hash FROM admin_users WHERE username = ?').get(username) as any;
    if (admin && await verifyPassword(password, admin.password_hash)) {
      return res.json({ token: signToken({ sub: admin.id, role: 'admin' }), role: 'admin' });
    }
    const team = db.prepare('SELECT id, username, password_hash FROM teams WHERE username = ?').get(username) as any;
  ...
  router.post('/auth/bootstrap-admin', async (req, res) => {
    const { username, password } = req.body as { username: string; password: string };
    const exists = db.prepare('SELECT COUNT(*) AS count FROM admin_users').get() as any;
    if (exists.count > 0) return res.status(400).json({ error: 'Admin already exists' });
  ...
  router.get('/team/:id/dashboard', requireRole('team'), (req, res) => {
    ...
    const news = (db.prepare('SELECT * FROM articles WHERE published = 1 ORDER BY created_at DESC LIMIT 6').all() as any[]).map(enrichArticleRow);
  ...
  router.get('/company/:id', (req, res) => {
    ...
    const profile = db.prepare('SELECT * FROM company_profiles WHERE company_id = ?').get(req.params.id) as any;
  ...
  router.get('/news', (_req, res) => {
    const articles = (db.prepare('SELECT * FROM articles ORDER BY pinned DESC, created_at DESC').all() as any[]).map(enrichArticleRow);
    res.json({ articles });
  });
  ...
  router.delete('/watchlist/:teamId/:companyId', requireRole('team'), (req, res) => {
    const auth = authFromReq(req);
    if (auth?.sub !== req.params.teamId) return res.status(403).json({ error: 'Forbidden' });
  ...
  router.post('/trade/buy', requireRole('team'), (req, res) => {
    const { teamId, companyId, quantity } = req.body as { teamId: string; companyId: string; quantity: number };
    const auth = authFromReq(req);
    ...
    const market = db.prepare('SELECT * FROM market_settings WHERE id = 1').get() as any;
    if (!company || !team || !quantity || quantity <= 0) return res.status(400).json({ error: 'Invalid trade' });
    if (auth?.sub !== teamId) return res.status(403).json({ error: 'Forbidden' });
    if (market.status === 'CLOSED' || market.status === 'PAUSED' || market.status === 'FROZEN') return res.status(400).json({ error: 'Trading disabled' });
    const maxAffordable = Math.floor(team.cash / company.price);
    const tradeQuantity = Math.min(quantity, maxAffordable);
    if (tradeQuantity <= 0) return res.status(400).json({ error: 'Insufficient cash' });
  ...
  router.post('/trade/sell', requireRole('team'), (req, res) => {
    ...
    const company = db.prepare('SELECT * FROM companies WHERE id = ?').get(companyId) as any;
    const team = db.prepare('SELECT * FROM teams WHERE id = ?').get(teamId) as any;
    const holding = db.prepare('SELECT * FROM holdings WHERE team_id = ? AND company_id = ?').get(teamId, companyId) as any;
    const market = db.prepare('SELECT * FROM market_settings WHERE id = 1').get() as any;
    if (!company || !team || !holding || !quantity || quantity <= 0) return res.status(400).json({ error: 'Invalid trade' });
    if (auth?.sub !== teamId) return res.status(403).json({ error: 'Forbidden' });
    if (market.status === 'CLOSED' || market.status === 'PAUSED' || market.status === 'FROZEN') return res.status(400).json({ error: 'Trading disabled' });
    const tradeQuantity = Math.min(quantity, holding.quantity);
    if (tradeQuantity <= 0) return res.status(400).json({ error: 'Insufficient shares' });
  ...
  router.post('/watchlist', requireRole('team'), (req, res) => {
    const { teamId, companyId } = req.body as { teamId: string; companyId: string };
    const auth = authFromReq(req);
    if (auth?.sub !== teamId) return res.status(403).json({ error: 'Forbidden' });
    const team = db.prepare('SELECT id FROM teams WHERE id = ?').get(teamId);
    const company = db.prepare('SELECT id FROM companies WHERE id = ?').get(companyId);
    if (!team || !company) return res.status(400).json({ error: 'Invalid watchlist entry' });
  ...
  router.post('/quiz/answer', (req, res) => {
    const { quizId, teamId, selectedIndex } = req.body as { quizId: string; teamId: string; selectedIndex: number };
    const quiz = db.prepare('SELECT * FROM quizzes WHERE id = ?').get(quizId) as any;
    if (!quiz || !quiz.active) return res.status(400).json({ error: 'Quiz inactive' });
  ...
  router.get('/admin/overview', requireRole('admin'), (_req, res) => {
    ...
    `).get(`-${activePresenceWindowMinutes()} minutes`) as { count: number };
  ...
  router.post('/admin/setup', async (req, res) => {
    const { adminUsername, adminPassword, startingCash, teams, companies, eventMode } = req.body as {
      adminUsername: string;
      adminPassword: string;
      startingCash: number;
      teams: Array<{ name: string; username: string; password: string }>;
      companies: Array<{ name: string; ticker: string; industry: string; description: string; sector: string; price: number; revenue: number; profit: number; debt: number; dividend_yield: number }>;
      eventMode: boolean;
    };
  ...
  router.post('/admin/market/status', requireRole('admin'), (req, res) => {
    const { status } = req.body as { status: 'OPEN' | 'CLOSED' | 'PAUSED' | 'FROZEN' };
  ...
  router.post('/admin/market/settings', requireRole('admin'), (req, res) => {
    const { inflation, interestRate, eventMode } = req.body as { inflation: number; interestRate: number; eventMode: boolean };
    ...
    if (inflation !== 0) {
      const companies = db.prepare('SELECT id, price FROM companies').all() as Array<{ id: string; price: number }>;
      for (const company of companies) {
  ...
  router.post('/admin/team', requireRole('admin'), async (req, res) => {
    ...
    const { id: teamId, name, username, password, cash, hidden } = req.body as { id?: string; name: string; username: string; password?: string; cash: number; hidden?: boolean };
    if (teamId) {
      db.prepare('UPDATE teams SET name = ?, username = ?, cash = ?, hidden = ? WHERE id = ?').run(name, username, cash, hidden ? 1 : 0, teamId);
      if (password) db.prepare('UPDATE teams SET password_hash = ? WHERE id = ?').run(await hashPassword(password), teamId);
  ...
  router.post('/admin/company', requireRole('admin'), (req, res) => {
    ...
    if (body.id) {
      ...
      const createdId = db.prepare('SELECT id FROM companies WHERE ticker = ?').get(body.ticker.toUpperCase()) as any;
      if (createdId?.id) {
  ...
  router.post('/admin/news', requireRole('admin'), (req, res) => {
    const body = req.body as { id?: string; title: string; body: string; category: string; impact_type: string; published?: boolean; companyIds?: string[] };
  ...
  router.post('/admin/quiz', requireRole('admin'), (req, res) => {
    const body = req.body as { id?: string; question: string; options: string[]; answerIndex: number; reward: number; active: boolean };
  ...
  router.post('/admin/notifications', requireRole('admin'), (req, res) => {
    const body = req.body as { teamId?: string; title: string; body: string; priority: number; pinned: boolean };
  ...
  router.post('/admin/trigger-event', requireRole('admin'), (req, res) => {
    ...
    const { name, targetType, targetValue, effectPct } = req.body as { name: string; targetType: string; targetValue: string; effectPct: number };
    ...
    const targetCompany = targetType === 'company' ? db.prepare('SELECT id, name, ticker FROM companies WHERE id = ?').get(targetValue) as { id: string; name: string; ticker: string } | undefined : undefined;
    ...
    db.prepare('INSERT INTO articles (id, title, body, category, impact_type, published) VALUES (?, ?, ?, ?, ?, 1)').run(
      id(),
      name || 'Market event',
      eventBody,
      'Market News',
      targetType === 'market' ? 'market' : targetType
    );
    const affectedCompanyIds = targetType === 'company'
      ? [targetValue]
      : targetType === 'sector'
        ? (db.prepare('SELECT id FROM companies WHERE sector = ?').all(targetValue) as Array<{ id: string }>).map((row) => row.id)
        : (db.prepare('SELECT id FROM companies').all() as Array<{ id: string }>).map((row) => row.id);
  ...
  router.post('/admin/backups/:fileName/restore', requireRole('admin'), (req, res) => {
    ...
    if (!safeName.endsWith('.json')) return res.status(400).json({ error: 'Only snapshot backups can be restored in-browser' });
    const backupRoot = path.resolve(config.backupDir);
    const filePath = path.resolve(backupRoot, safeName);
    if (!filePath.startsWith(backupRoot)) return res.status(400).json({ error: 'Invalid backup file' });
  ...
  router.get('/exports/portfolios.csv', requireRole('admin'), (_req, res) => {
    const rows = leaderboardRows().map((row: any) => ({ team_name: row.name, cash: row.cash, portfolio_value: row.portfolio_value, net_worth: row.net_worth }));
    res.type('text/csv').send(csvFromRows(rows));
  });
  ...
  router.post('/admin/team/:id/cash', requireRole('admin'), (req, res) => {
    const { cash } = req.body as { cash: number };
  ...
  router.post('/admin/team/:id/notification', requireRole('admin'), (req, res) => {
    const { title, body, priority, pinned } = req.body as { title: string; body: string; priority: number; pinned: boolean };
  ...
  router.post('/admin/company/:id/dividend', requireRole('admin'), (req, res) => {
    const { dividend_yield } = req.body as { dividend_yield: number };
  ...
  router.get('/admin/quizzes', requireRole('admin'), (_req, res) => {
    const quizzes = db.prepare('SELECT * FROM quizzes ORDER BY created_at DESC').all().map((quiz: any) => ({ ...quiz, options: JSON.parse(quiz.options_json) }));
    res.json({ quizzes });
  });

  router.get('/admin/news', requireRole('admin'), (_req, res) => {
    res.json({ articles: (db.prepare('SELECT * FROM articles ORDER BY created_at DESC').all() as any[]).map(enrichArticleRow) });
  });
  ...
  router.post('/therapist/explain', requireRole('team'), async (req, res) => {
    const { mode, finding } = req.body as { mode?: string; finding?: Record<string, any> };
    if (!config.geminiApiKey) return res.status(503).json({ error: 'AI explanation is not configured' });
    ...
    if (!response.ok) return res.status(502).json({ error: 'AI explanation failed' });
    const data: any = await response.json();
    res.json({ text: data.candidates?.[0]?.content?.parts?.map((part: any) => part.text ?? '').join('').trim() });
  });

  router.post('/therapist/speak', requireRole('team'), async (req, res) => {
    const text = String(req.body?.text ?? '').trim();
    if (!config.elevenLabsApiKey) return res.status(503).json({ error: 'Voice is not configured' });
    if (!text || text.length > 800) return res.status(400).json({ error: 'Invalid text' });
    ...
    if (!response.ok) return res.status(502).json({ error: 'Voice generation failed' });
