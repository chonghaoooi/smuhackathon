...
export function initDb() {
  ...
  const profileColumns = db.prepare('PRAGMA table_info(company_profiles)').all() as Array<{ name: string }>;
  ...
  const presenceColumns = db.prepare('PRAGMA table_info(session_presence)').all() as Array<{ name: string }>;
